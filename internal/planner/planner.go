		...
		if orderDir.HasValue() && p.joinExpand.exhaustive {
			if !p.joinExpand.inNestedJoin {
				...
				if join.parentSide.isPrimary() {
					...
					// Secondary parent: orphans identified by point lookups on child's FK index.
...
// into the selectTopNode for secondary-side orphan handling via point lookups.
// Orphans are identified by iterating parents and checking the child's FK index.
// Called after the full plan chain (order, limit) is built.
